...
#' @param tau The tau (or \eqn{\tau}) option allows researchers to specify the 'time delay',
#' which essentially sorts the data by the multiple \eqn{\tau}. This is done by specifying lagged embeddings
#' that take the form: \eqn{t,t-\tau,…,t-(E-1)\tau}, where the default is tau(1) (i.e., typical lags). However, if
...
#' sizes at small values of L can be used, such as if \eqn{E = 2} and \eqn{T = 100}, with the setting then perhaps
...
#' default value. When \eqn{k < 0} (e.g., k(-1)), all possible points in the prediction set are used (i.e.,
...
#' may be computationally burdensome and therefore k(100) or perhaps k(500) may be preferred if \eqn{T} or \eqn{NT}
#' is large.
#' 
...
#' algorithm(smap) invokes something analogous to a distributed lag model with \eqn{E + 1} predictors
...
#' 
#' @param saveSMAPCoeffs  This option allows smap coefficients to be stored in variables with
...
#' 
#' @param reldt This option, to be read as 'relative dt', is like the 'dt' option above in
...
#' variable.
#' 
#' @param numReps The explore subcommand uses a random 50/50 split for simplex
