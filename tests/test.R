...
chicago <- read_csv("~/Dropbox/EDM/fastEDM/tests/chicago.csv", col_types = cols(crime = col_double()))
#chicago <- read_csv("chicago.csv", col_types = cols(crime = col_double()))


opts <- list(saveInputs = "testchicago.json", k = -1)
...
res <- run_command(df, es, libs, opts, tau, p, numReps=10, verbosity=5)
cat("finished chicago test")

library(jsonlite)
parsed <- fromJSON(res)

maes <- unlist(lapply(parsed$stats, function(x) { x$mae } ))
rhos <- unlist(lapply(parsed$stats, function(x) { x$rho } ))
...
# library(tidyjson)
# 
# jres <- as.tbl_json(res)
# 
# jres <- res %>%            # Use the %>% pipe operator to pass json through a pipeline 
...
#     rho = jnumber("rho"),  # Extract the "name" object as a character column "user.name"
#     mae = jnumber("mae")     # Extract the "age" object as a numeric column "user.age"
#   )
# 
# plot(jres[["rho"]])
# 
# colnames(df) <- c("t", "y", "x")
# 
# res2 <- run_command(df, es, libs, opts, tau, p, verbosity=5) #, 0, 0, FALSE, FALSE, FALSE)
# 
# 
# jres2 <- res %>%            # Use the %>% pipe operator to pass json through a pipeline 
#   as.tbl_json %>%   # Parse the JSON and setup a 'tbl_json' object
#   #gather_array %>%  # Gather (stack) the array by index
#   enter_object("stats") %>%
#   gather_array %>%
